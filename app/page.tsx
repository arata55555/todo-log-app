    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 md:gap-10">
      <div className="lg:col-span-7 space-y-6">
        <div className="flex items-center justify-between bg-white p-2 rounded-2xl md:rounded-[2rem] shadow-xl shadow-slate-200/50 h-16 md:h-20 px-3 md:px-6">
          <Button variant="ghost" className="h-10 w-10 md:h-14 md:w-14 rounded-full" onClick={() => setDate(subDays(date, 1))} aria-label="前"><ChevronLeft /></Button>
          ...
          <Button variant="ghost" className="h-10 w-10 md:h-14 md:w-14 rounded-full" onClick={() => setDate(addDays(date, 1))} aria-label="次"><ChevronRight /></Button>
        </div>
        ...
        <div className="p-4 md:p-8 bg-indigo-600 rounded-3xl md:rounded-[3rem] shadow-2xl shadow-indigo-200 space-y-4 md:space-y-6">
          ...
          <div className="flex flex-wrap gap-2">
            {templates.map(tmpl => <button key={tmpl.id} onClick={() => addTask(tmpl.title, tmpl.scheduled_start, tmpl.scheduled_end)} className="bg-indigo-500/50 border border-indigo-400/30 text-white px-3 py-2 rounded-xl font-black text-[10px] active:scale-95 transition-all flex items-center"><Plus className="w-3 h-3 mr-1" /> {tmpl.title}</button>)}
          </div>
        </div>
          ...
          <CardContent className="p-4 md:p-6 space-y-4">
            ...
            <div className="flex flex-col sm:flex-row gap-3">
              <div className="flex gap-2 flex-1">
                <Input type="time" className="h-12 md:h-14 flex-1 rounded-2xl border-none bg-slate-100/50 px-4 font-bold text-center" value={startTime} onChange={(e) => setStartTime(e.target.value)} aria-label="開始時間" />
                <Input type="time" className="h-12 md:h-14 flex-1 rounded-2xl border-none bg-slate-100/50 px-4 font-bold text-center" value={endTime} onChange={(e) => setEndTime(e.target.value)} aria-label="終了時間" />
              </div>
        ...
        <div className="space-y-3">
          {tasks.map(t => (
            <div key={t.id} className="group flex flex-col md:flex-row md:items-center p-4 md:p-6 bg-white border-none rounded-[2rem] shadow-sm">
              <div className="flex items-center flex-1">
                <Checkbox checked={t.is_completed} className="w-6 h-6 rounded-lg border-slate-200" onCheckedChange={async () => { await supabase.from("tasks").update({ is_completed: !t.is_completed }).eq("id", t.id); fetchData(); }} />
              ...
              <div className="flex items-center justify-between mt-3 md:mt-0 md:ml-6 gap-3">
                <div className="flex items-center gap-1 bg-slate-50 p-2 rounded-xl border border-slate-100">
                  <input type="time" value={t.scheduled_start.slice(0, 5)} onChange={async (e) => { await supabase.from("tasks").update({ scheduled_start: e.target.value }).eq("id", t.id); fetchData(); }} className="bg-transparent text-[10px] font-black text-indigo-500 w-14 text-center focus:outline-none" aria-label="開始" />
                  <span className="text-slate-300 text-[10px]">-</span>
                  <input type="time" value={t.scheduled_end.slice(0, 5)} onChange={async (e) => { await supabase.from("tasks").update({ scheduled_end: e.target.value }).eq("id", t.id); fetchData(); }} className="bg-transparent text-[10px] font-black text-indigo-500 w-14 text-center focus:outline-none" aria-label="終了" />
                </div>
    ...
    <div className="max-w-4xl mx-auto space-y-6 md:space-y-10">
      <Card className="bg-white border-dashed border-2 md:border-4 rounded-3xl md:rounded-[3rem] border-slate-100 shadow-sm overflow-hidden">
        ...
        <CardContent className="p-5 md:p-10 space-y-4 md:space-y-6">
          <Input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Routine name..." className="h-12 md:h-16 rounded-2xl text-base md:text-xl border-none bg-slate-50 px-4 md:px-8 font-bold" aria-label="テンプレート名" />
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="text-[8px] font-black text-slate-400 mb-1 block uppercase font-bold">Start</label>
                <Input type="time" value={start} onChange={(e) => setStart(e.target.value)} className="h-12 md:h-16 rounded-2xl border-none bg-slate-50 font-black text-indigo-600 text-center" aria-label="開始時間" />
              </div>
              <div>
                <label className="text-[8px] font-black text-slate-400 mb-1 block uppercase font-bold">End</label>
                <Input type="time" value={end} onChange={(e) => setEnd(e.target.value)} className="h-12 md:h-16 rounded-2xl border-none bg-slate-50 font-black text-indigo-600 text-center" aria-label="終了時間" />
              </div>
            </div>
        ...
        {templates.map(tmpl => (
          <div key={tmpl.id} className="flex flex-col p-4 md:p-6 bg-white border-none rounded-3xl shadow-sm">
            <div className="flex justify-between items-start mb-4">
              ...
              <Button variant="ghost" size="icon" className="text-slate-100 hover:text-red-500 rounded-full" onClick={async () => { await supabase.from("task_templates").delete().eq("id", tmpl.id); fetchTemplates(); }} aria-label="削除"><Trash2 className="w-4 h-4" /></Button>
            </div>
            <div className="flex items-center gap-2 bg-indigo-50/50 p-3 rounded-2xl border border-indigo-100/50 w-full justify-center">
              ...
              <span className="text-indigo-200 font-black text-xs tracking-widest">TO</span>
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
      <Card className="bg-white shadow-2xl shadow-slate-200/50 border-none rounded-3xl md:rounded-[2.5rem] overflow-hidden">
        ...
        <div className="grid grid-cols-7 border-b bg-slate-50/20">
          {/* ↓ ここがエラーの原因箇所：key={i} を追加してユニークにします ↓ */}
        ...
        <div className="grid grid-cols-7 auto-rows-[minmax(60px,auto)] md:auto-rows-[minmax(120px,auto)]">
          {calendarDays.map((day, i) => {
            ...
            return (
              <div key={i} onClick={() => { setViewDate(day); setTab("day"); }} className={`p-1 md:p-2 border-r border-b border-slate-100 relative cursor-pointer group transition-all ${cellBg}`}>
